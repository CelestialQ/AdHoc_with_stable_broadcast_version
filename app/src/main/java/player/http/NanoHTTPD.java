...
public class NanoHTTPD
{
	...
	public void start() throws IOException {
		Log.d("NanoHTTPD", "in NanoHTTPD start");
		...
		myThread = new Thread( new Runnable()
		{
			public void run()
			{
				...
				{
					while( true ){
						Log.d("NanoHTTPD", "new HTTPSession");
...
*/
	...
	private class HTTPSession implements Runnable
	{
		...
		{
			Log.d("NanoHTTPD", "in sendResponse");
			try
			{
				if ( status == null )
				...
				if ( data != null )
				{
					...
					while (pending>0)
					{
						Log.d("NanoHTTPD", "pending : "+pending);
						Log.d("NanoHTTPD", "read from here : " +  ( (pending>theBufferSize) ?  theBufferSize : pending ));
			...
			boolean allowDirectoryListing )
	{
		...
		try
		{
			if ( res == null )
			{
				...
				long fileLen = 273888220;
				if (startFrom == 273350656) {
				...
				if (range != null && startFrom >= 0)
				{
					...
					else
					{
						...
						CacheInputStream fis = new CacheInputStream();
						//skip is here 
						fis.skip( startFrom );
				...
				else
				{
					...
					{
						res = new Response( HTTP_OK, mime, new CacheInputStream());
	...
	private static int theBufferSize = CacheConfiguration.blocksize;
