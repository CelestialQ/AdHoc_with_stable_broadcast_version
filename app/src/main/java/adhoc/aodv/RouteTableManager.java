...
public class RouteTableManager {
	...
	private final static String MSG_TAG = "AdHoc --> RouteTableManager";
	...
	public RouteTableManager(int nodeAddress, Node parent) {
		...
		fileName = Environment.getExternalStorageDirectory().getPath() + "/AdhocRoutePath_" + nodeAddress + "_" + time + ".txt";
	...
	protected boolean createForwardRouteEntry(int destinationNodeAddress, int nextHopAddress,
		...
		if (forwardRouteTable.addForwardRouteEntry(forwardRouteEntry)) {
			...
			if(notifyObserver){
				...
				newPath = nodeAddress + "," + forwardRouteEntry.getDestinationAddress() + "," + forwardRouteEntry.getNextHop() + newLine;
				try {
