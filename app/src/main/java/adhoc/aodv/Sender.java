...
public class Sender implements Runnable{
	...
	private final static String MSG_TAG = "AdHoc --> Sender";
    ...
    private Queue<UserBroadcastDataPacket> userBroadcastMessagesToForward;
//    private Queue<UserBroadcastDataPacket> userBroadcastMessagesFromNode;
    ...
    public Sender(Node parent,int nodeAddress, RouteTableManager routeTableManager) throws SocketException, UnknownHostException, BindException {
        ...
        userBroadcastMessagesToForward = new ConcurrentLinkedQueue<UserBroadcastDataPacket>();
//        userBroadcastMessagesFromNode = new ConcurrentLinkedQueue<UserBroadcastDataPacket>();
    	...
    	while(keepRunning){
        	try {
	        	synchronized(queueLock){
	    			while(pduMessages.isEmpty() && userMessagesToForward.isEmpty() && (isRREQsent || userMessagesFromNode.isEmpty()) && userBroadcastMessagesToForward.isEmpty()){
	    				queueLock.wait();
	    			}
	    		}
	        	...
	        	if(!isRREQsent){ 
		        	if(!userMessagesFromNode.isEmpty()){
			    		...
			    		while(userData != null){
			    			try{
			    				Log.d("BROADCAST", "Before send in Sender.");
								if(!sendUserDataPacket(userData)){
									Log.d("BROADCAST", "Send failure.");
									isRREQsent = true;
...
//										System.out.println("stop time : " + stopTime + " // start time : " + startTime + " // total size : " + totalSize);
//										speed = totalSize / (stopTime - startTime);
//										//更新TextView
//										System.out.println("send " + speed + " kb/s");
//										startTime = stopTime;
//										interval = 0;
//										totalSize = 0;
//									}
									Log.d("BROADCAST","Send success.");
								/*
									Log.d("BROADCAST", "send src node: "+userData.getSourceNodeAddress());
									Log.d("BROADCAST", "send forward node: "+userData.getForwardAddress());
									Log.d("BROADCAST", "send des node: "+userData.getDestinationAddress());
									Log.d("BROADCAST", "send packetId: "+userData.getPacketID());
									Log.d("BROADCAST", "send dataType: "+userData.getDataType());
									Log.d("BROADCAST", "send datalength: "+userData.getData().length);*/
									
									parent.notifyAboutDataSentSucces(userData.getPacketID());
//									interval ++;
//									totalSize = totalSize + userData.getData().length;
								}
	    		...
	    		UserBroadcastDataPacket broadcastPacket = userBroadcastMessagesToForward.peek();
	    		while(broadcastPacket != null) {
	    			try {
	    			...
	    			userBroadcastMessagesToForward.poll();
	    			broadcastPacket = userBroadcastMessagesToForward.peek();
	    		}
	...
	private boolean sendUserDataPacket(UserDataPacket packet) throws DataExceedsMaxSizeException, InvalidNodeAddressException{
		if(		packet.getDestinationAddress() != Constants.BROADCAST_ADDRESS
			...
			try {
				...
				packet.setForwardAddress(nextHop);
			//	Log.d("BROADCAST", nextHop + "/" + nodeAddress);
				try {
					if (packet.getDataType() == (byte)0) {
						sendFlag = udpSender.sendPacketUnicast(nextHop, packet.toBytes());
					}
					else if(packet.getDataType() == (byte)1){
						sendFlag = udpSender.sendPacketSpecificBroadcast(packet.toBytes());
					}
					
				//	Log.d("BROADCAST", "Before Send in sendUserDataPacket() @Sender.");
				//	sendFlag = udpSender.sendPacketUnicast(destinationNodeID, data)(packet.toBytes());
					if(sendFlag) {
				//		Log.d("BROADCAST", "SSSSSSSS!");
						//记录 id,srcAddress,nodeAddress,nextHop,desAddress;
						//parent.recordPath(packet.getPacketID(), packet.getSourceNodeAddress(), packet.getDestinationAddress(), nextHop);
					}else{
				//		Log.d("BROADCAST", "FFFFFFFF!");
					}
					return sendFlag;
				} catch (IOException e) {
				//	Log.d("BROADCAST","IOEXCEPTION.");
    ...
    protected void queueUserBroadcastMessageToForward(UserBroadcastDataPacket userData){
    	userBroadcastMessagesToForward.add(userData);
    	synchronized (queueLock) {
			queueLock.notify();
		}
    }
...
//    protected void queueUserBroadcastMessageFromNode(UserBroadcastDataPacket userPacket){
//    	userBroadcastMessagesFromNode.add(userPacket);
//    	synchronized (queueLock) {
//    		queueLock.notify();
//		}
//    }
